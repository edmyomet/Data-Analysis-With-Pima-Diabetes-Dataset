...
class Diabetes:
    def __init__(self):
        self.df = pd.read_csv(r'.\datasets\diabetes.csv')
        self.feature_list = self.df.columns
        
    def __dataset_description(self):
        self.df.describe().to_csv(r'.\output\results\dataset_description.csv')

    def __freq(self):
        for feature in self.feature_list:
            pd.DataFrame(self.df[feature].value_counts()).to_csv(r'.\output\results\freq.csv',mode='a')
            
    def __mean(self):
        self.mean = self.df.mean(numeric_only=True,skipna=True)
        self.mean.to_csv(r'.\output\results\mean.csv')
    def __median(self):
        self.median = self.df.median(numeric_only=True,skipna=True)
        self.median.to_csv(r'.\output\results\median.csv')
        
    def __mode(self):
        self.mode = self.df.mode(numeric_only=True)
        self.mode.to_csv(r'.\output\results\mode.csv')
    ...
    def __boxplot(self):
        ...
        plt.savefig(r'.\output\img\boxplot.png')
    
    def __std(self):
        self.std = self.df.std(numeric_only=True)
        self.std.to_csv(r'.\output\results\std.csv')
    ...
    def __normal_distr(self):
        ...
        plt.savefig(r'.\output\img\histplot.png')
        
    def __skewness(self):
        self.skew = self.df.skew(numeric_only=True)
        self.skew.to_csv(r'.\output\results\skew.csv')
    
    def __kurtosis(self):    
        self.kurt = self.df.kurtosis(numeric_only=True)
        self.kurt.to_csv(r'.\output\results\kurtosis.csv')
    ...
    def __cov(self):
        ...
        self.cov.to_csv(r'.\output\results\cov.csv')
    
    def __corr(self):
        self.corr = self.df.corr(numeric_only=True)
        self.corr.to_csv(r'.\output\results\corr.csv')
    ...
    def __scatter(self):
        ...
        for i in range(8):
            for j in range(8):
                ...
                sns.scatterplot(x=x,y=y,hue=label,palette='flare',alpha=0.7,ax=axes[i][j])
                axes[i][j].plot(x,m*x+b,linewidth=2,color='black')
        plt.savefig(r'.\output\img\scatterplot.png')
    ...
    def __heatmap(self):
        ...
        plt.savefig(r'.\output\img\heatmap.png')    
    ...
    def __MinMax_Scaler(self):
        ...
        pd.DataFrame(self.df_scaled,columns=self.df.columns).to_csv(r'.\datasets\cleaned_dataset.csv')
    
    def __std_scaler(self):
        self.scaler = StandardScaler()
        self.df_scaled = self.scaler.fit_transform(self.df)
        pd.DataFrame(self.df_scaled,columns=self.feature_list).to_csv(r'.\datasets\cleaned_dataset.csv')       
    def analysis(self):
